...
COPY . .
RUN npm run build
RUN rm -rf node_modules src public && npm i -g serve
EXPOSE 3000
CMD ["serve", "-s", "dist"]
...
<ul style={{ paddingLeft:'40px', listStyleType: 'dot' }}>
    ...
    <li>Removing the files that are not needed after the build process and installing serve package for serving our dist folder, which contains the html, css and javascript after building the application</li>
...
<br/>
Note: This is enough to build the image, also we have removed the files that are not needed, to reduce the size of docker image.
